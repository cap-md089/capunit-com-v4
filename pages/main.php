...
rightsection;
            ...
            $stmt = $pdo->prepare("SELECT EventNumber FROM ".DB_TABLES['EventInformation']." WHERE MeetDateTime > :now AND Status!='Draft' AND AccountID = :aid AND ShowUpcoming = 1 ORDER BY MeetDateTime ASC LIMIT :limit;");
            $stmt->bindValue(':now', time());
			$stmt->bindValue(':aid', $a->id);
